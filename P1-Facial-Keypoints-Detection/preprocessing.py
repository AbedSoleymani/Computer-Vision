...
"""
We will write each preprocessing step as callable classes
instead of simple functions so that parameters of the
transform need not be passed everytime it's called.

Such method has tateful behavior, i.e., lasses can maintain
internal state using instance variables. This means that
instances of the class can remember information across multiple
calls. Functions, on the other hand, do not have built-in state
unless you use global variables or closures.
"""      

class Normalize(object):
    """
    It inherits from the base class `object`, which is
    the default parent class for all Python classes!
    """

    """
    Normalize the color range to [0,1] and
    convert a color image to grayscale if needed
    """        
...
class Rescale(object):
    ...
    def __call__(self, sample):
        ...
        # We have to scale the facial key points, too
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        # numpy image: H x W x C (Height, Width, Channel)
